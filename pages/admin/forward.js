...
export default function Forward(){
  ...
  const [form,setForm]=useState({ clientName:'', phone:'', address:'', techId:'' });
  ...
  async function submit(e){
    e.preventDefault();
    const r = await fetch('/api/admin/forward', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(form) });
    const d = await r.json();
    if(!r.ok){ toast.error(d.error||'Failed'); return; }
    toast.success('Call forwarded');
    setForm({ clientName:'', phone:'', address:'', techId:'' });
  }
      ...
      <main className="max-w-3xl mx-auto p-4 space-y-3">
        <div className="card">
          ...
          <form onSubmit={submit} className="grid gap-2">
            ...
            <input className="input" placeholder="Client Phone Number" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} required />
            <input className="input" placeholder="Address" value={form.address} onChange={e=>setForm({...form,address:e.target.value})} required />
            <select className="input" value={form.techId} onChange={e=>setForm({...form,techId:e.target.value})} required>
