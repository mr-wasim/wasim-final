...
async function handler(req, res, user) {
  ...
  try {
    ...
    // determine start/end (dateFrom/dateTo override month)
    let start, end;
    if (dateFrom || dateTo) {
    ...
    const monthStatsArr = await callsColl.aggregate([
      ...
      { $group: { _id: "$techId", monthClosed: { $sum: 1 }, monthAmount: { $sum: "$priceNum" }, monthEmbeddedPaid: { $sum: { $reduce: { input: "$embeddedPayments", initialValue: 0, in: { $add: ["$$value", { $ifNull: ["$$this.amount", 0] }] } } } } } }
    ]).toArray();
    ...
    const paymentsMonthAgg = await paymentsColl.aggregate([
      { $match: paymentsMatch },
      { $group: { _id: null, online: { $sum: { $ifNull: ["$onlineAmount", 0] } }, cash: { $sum: { $ifNull: ["$cashAmount", 0] } }, total: { $sum: { $add: [{ $ifNull: ["$onlineAmount", 0] }, { $ifNull: ["$cashAmount", 0] }] } } } }
    ]).toArray();
    ...
    if (techCandidates.length) {
      const callsPipeline = [
        ...
        // lookup payments in the same payment-range (payments.createdAt in [start,end))
        {
          $lookup: {
            from: "payments",
        ...
        // flatten matched calls inside payments
        {
          $project: {
            _id: { $toString: "$_id" },
        ...
        { $addFields: { submittedAmount: { $reduce: { input: "$matchedCalls", initialValue: 0, in: { $add: ["$$value", { $add: ["$$this.onlineAmount", "$$this.cashAmount"] }] } } }, lastPaymentAt: { $max: "$matchedCalls.paymentCreatedAt" } } },

        { $project: { _id: 1, techId: 1, clientName: 1, customerName: 1, phone: 1, address: 1, type: 1, serviceType: 1, status: 1, price: 1, createdAt: 1, closedAt: 1, closedDate: 1, submittedAmount: 1, matchedPayments: "$matchedCalls", paymentStatus: { $cond: [{ $gt: ["$submittedAmount", 0] }, "Submitted", "Unsubmitted"] }, lastPaymentAt: 1, clientAvatar: { $ifNull: ["$clientAvatar", "$avatar"] }, avatar: 1 } },
    ...
    // payments flattened (based on payments.createdAt in range) — matches payment.js
    ...
    return res.status(200).json({ success: true, technicians, summary, calls, payments: paymentsList, monthSummaryByStatus });
  } catch (err) {
